...
def compute_bbdd_surf_descriptors(image, threshold=400):
    #Find the SURF keypoints and descriptors of a given image
    im = cv.imread(image)

    return surf_descriptor(im, threshold, False)

def surf_descriptor(painting, threshold=400, bounding_rm=True):
    #Find the SURF keypoints and descriptors of a given image
    img_gray = cv.cvtColor(painting, cv.COLOR_BGR2GRAY)
    surf = cv.xfeatures2d.SURF_create(threshold, )
    if bounding_rm:
        bounding = text_boxes.detect_text_box(painting)
        ...
        kp, des = surf.detectAndCompute(img_gray, mask=bounding_mask)
    else:
        kp, des = surf.detectAndCompute(img_gray, mask=None)
...
def match_descriptors(bbdd_kp_des, query_des, method='ORB'):
    ...
    if method == 'SURF':
        ...
        metric = cv.NORM_HAMMING
    bd_kp, bd_des = bbdd_kp_des
    if len(bd_kp) > 0:
        # create BFMatcher object
        bf = cv.BFMatcher(metric, crossCheck=True)
        # Match descriptors.
